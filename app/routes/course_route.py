...
@course_bp.route('/delete_course/<string:coursecode>', methods=['DELETE'])
def delete_course(coursecode):
    try:
        # Call the delete_course function from course_models.py
        success = course_model.delete_course(coursecode)
        if success:
